  ...
  await fetch(`${backendURL}/users/me/orders`, {
    ...
    .then(async (res) => {
      ...
      response.forEach((order) => {
        ...
        parent.style =
          "display: flex; align-items: center; justify-content: center; flex-direction: column; margin-top: 20px; text-align: center;";
        parent.id = order.orderId;
        tableDiv.appendChild(parent);

        const tableId = document.createElement("h1");
        tableId.style =
          "text-align: center; margin-top: 20px; font-size: 30px; color: #000;";
        ...
        order.items.forEach((item) => {
          ...
          card.className = "card";
          card.style =
            "background-color: #fff; border-radius: 10px; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1); padding: 10px; margin: 10px; display: flex; flex-direction: column; align-items: center; max-width: 200px; hover: transform: scale(1.05); transition: transform 0.3s;";
          ...
          price.textContent =
            item.plate.Price.toFixed(2) +
            " EGP" +
            " x " +
            item.quantity +
            " = " +
            (item.plate.Price * item.quantity).toFixed(2) +
            " EGP";
